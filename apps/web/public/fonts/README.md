...
⚠️ **注意**: 字体文件已迁移到 `apps/web/src/app/fonts/` 目录

Next.js `localFont` 要求字体文件路径相对于源文件，因此：
- ✅ **新位置**: `apps/web/src/app/fonts/`（正确）
- ❌ **旧位置**: `apps/web/public/fonts/`（已废弃）

请将字体文件下载到 `apps/web/src/app/fonts/` 目录。

---

## 旧说明（仅供参考）

此目录原本用于存放本地字体文件，避免构建时依赖 Google Fonts。
